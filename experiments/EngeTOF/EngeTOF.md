...
*図2. 補正によるadc-adc分布の変化*

![補正によるadc-adc分布の変化](images/etoftest2026_run_47_timewalk.png)

## 4. 考察
- 1X5-2X4:{4,5,10,11}の組み合わせだけ元の分解能が悪すぎたりデータ数が半分以下しかなかったりと怪しい。ここが160psくらいまで抑えられれば上の話も辻褄が合うのでは。本当は$`\sigma_{1X5}^2 - \sigma_{1X4}^2`$が0になってくれると「上段と下段それぞれで分解が一緒」という仮定が正しいことになるのでそう合ってほしい。cut条件が甘い？
